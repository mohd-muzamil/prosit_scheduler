...
#     print("loop...")
#     for start_str in locker(gen(), dict__url=config.get("Run", "stepCountersinceLastReboot"), timeout=180):  # 3min.
#         start = datetime.strptime(start_str, "%Y-%m-%d")
#         pids = db["all"].distinct("participantId", bydate(start=start, days=1, attribute="stepCounter_sinceLastReboot"))
#         for pid in pids:
#             print("     ", pid)
#             docs = db["all"].aggregate([
#                 {"$match": bydate(start=start, days=1, attribute="stepCounter_sinceLastReboot", participantId=pid)},
#                 {"$sort": {"measuredAt": 1}},
#                 {"$project": {"_id": False}}
#             ])
#
#             newdocs = []
#             steps_since_reboot0 = None
#             for doc in docs:
#                 steps_since_reboot = float(doc["value"])
#                 start_t = v[0]
#                 stop_t = v[1]
#                 fmt = "%Y-%m-%d %H:%M:%S"
