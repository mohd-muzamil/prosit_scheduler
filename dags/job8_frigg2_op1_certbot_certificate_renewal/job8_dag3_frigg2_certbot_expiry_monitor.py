...
dag = DAG(
    dag_id='frigg2_certbot_expiry_monitor',
    default_args=default_args,
    schedule_interval='@daily',
    catchup=False,
    tags=['certbot', 'monitoring', 'frigg2'],
)
...
def parse_certbot_output(ti):
    ...
    if days_left < alert_threshold:
        subject = f"[ALERT] frigg2 SSL Certificate expiring in {days_left} days"
        body = f"The SSL certificate for frigg2-mc expires on {expiry_date}.\n\nCertbot output:\n\n{decoded_output}"
